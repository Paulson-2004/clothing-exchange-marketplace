...
- **Middleware** (`authMiddleware.js`):
  ...
  - `requireAdmin` — must run after `protect`; checks `req.user.role === 'admin'`, 403 otherwise. **Currently unused by any route** (no admin endpoints exist yet).
...
- **Role assignment**: `role` is **never** accepted from the client on registration — it always defaults to `'user'`. The only way to create an admin is running `npm run seed:admin` with `ADMIN_EMAIL`/`ADMIN_PASSWORD` env vars set, which is a manual, deliberate step never triggered automatically.
...
No `/api/admin` mount exists yet — the comment in `app.js` (`// More route groups (admin) will be mounted here in later phases.`) is the only trace of that intention.
...
- **Routing**: single `BrowserRouter` in `App.jsx`, `ProtectedRoute` wrapper redirects unauthenticated users to `/login` (waits for `AuthContext`'s `initializing` flag before deciding, to avoid a false redirect flash on page refresh).
